...
class GlobalUtils {
    ...
    isPathExistsError(targetPath) {
        ...
        if (!targetPath) {
            throw new Error(`targetPath not received: ${targetPath} (1000019)`);
        }
        // Check if the path parameter exists.
        if (!fs.existsSync(targetPath)) {
            throw new Error(`targetPath not exists: ${targetPath} (1000020)`);
        }
    }
    ...
    isPathAccessible(targetPath) {
        ...
        if (fs.accessSync(targetPath, fs.constants.R_OK)) {
            throw new Error(`targetPath not readable: ${targetPath} (1000021)`);
        }
        // Check if the path is writable.
        if (fs.accessSync(targetPath, fs.constants.W_OK)) {
            throw new Error(`targetPath not writable: ${targetPath} (1000022)`);
        }
    }
}
