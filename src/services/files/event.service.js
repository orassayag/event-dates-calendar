import { dictionaryCulture, eventCulture } from '../../culture/index.js';
import {
  CalendarDayModel,
  CommonTaskModel,
  EventDateModel,
...
import { EventTypeEnum } from '../../core/enums/index.js';
...
class EventService {
  ...
  async createEventDates() {
    ...
    const missingCalendarEventDates =
      this.createMissingCalendarEventDates(calendarILEventDates);
    ...
    const {
      ...
      endYearTasks,
      futureTasks,
    } = await this.createSourceEventDates();
    // Next, create the calendar days to log.
    const calendarDaysList = this.createCalendarDays(
      [
        ...calendarILEventDates,
        ...missingCalendarEventDates,
        ...calendarUSEventDates,
        ...staticEventDates,
        ...sourceEventDates,
      ],
      futureTasks
  ...
  createTextEvent(event) {
    if (!!event.eventYear || event.eventYear === 0) {
      event.text = `${event.text} (${Math.abs(
        parseInt(new Date().getFullYear() - event.eventYear, 10)
      )}).`;
    }
    return event;
  }
  ...
  async createILCalendarEventDates() {
    ...
    const dom = await domUtils.getDOMFromURL(
      applicationService.applicationDataModel.calendarILLink
    );
    const daysList = dom.window.document.querySelectorAll(
      `.${separatorService.dayInMonthDOM},.${separatorService.todayDOM}`
    );
    ...
    for (let i = 0; i < daysList.length; i++) {
      ...
      const spansDOMList = dayDOM.getElementsByTagName(
        separatorService.spanDOM
      );
      if (spansDOMList.length > 1) {
        // Example of day Id: id20211214.
        const dayIdDOM = dayDOM
          .getElementsByClassName(separatorService.personalDOM)[0]
          .getAttribute(separatorService.idDOM);
        ...
        for (let y = 1; y < spansDOMList.length; y++) {
          const event = this.createEventDate({
            ...
            text: eventUtils.createEventTemplate(
              this.replaceEvents(
                replaceEventsDates,
                spansDOMList[y].textContent
              )
            ),
          });
          calendarILEventDates.push(event);
          if (event.isVacation) {
            calendarILEventDates.push(
  ...
  async createUSCalendarEventDates() {
    ...
    const holidaysList = dom.window.document.getElementsByTagName(
      separatorService.rowDOM
    );
    for (let i = 0; i < holidaysList.length; i++) {
      ...
      if (cell && cell.textContent) {
        ...
        const event = dynamicEventDates.find(
          (e) => e.includeText === holidayName
        );
        if (event) {
          const dateUnix = holidayDOM.getAttribute(
            separatorService.unixDataDOM
          );
          const date = timeUtils.getCurrentDate(parseInt(dateUnix));
          const { dateDay, dateMonth, dateYear } = timeUtils.getDateParts(date);
          calendarUSEventDates.push(
            this.createEventDate({
              day: dateDay,
              month: dateMonth,
              year: dateYear,
  ...
  createMissingCalendarEventDates(calendarEventDates) {
    ...
    for (let i = 0; i < missingCalendarEventDates.length; i++) {
      const { includeText, excludeText, displayText, isDayBefore } =
        missingCalendarEventDates[i];
      const calendarEventDate = calendarEventDates.find((e) => {
        return (
          e.text.indexOf(includeText) > -1 &&
          e.text.indexOf(
            excludeText ? excludeText : separatorService.toggleSeparator
          ) === -1
        );
      });
      if (calendarEventDate) {
        const { day, month, year, text } = calendarEventDate;
        resultMissingCalendarEventDates.push(
          this.createEventDate({
            ...
            eventType: EventTypeEnum.CALENDAR,
            text: displayText
              ? eventUtils.createEventTemplate(displayText)
              : eventUtils.completeEventTemplate(
                  dictionaryCulture.eveNight,
                  text
                ),
          })
        );
  ...
  createStaticEventDates() {
    ...
    for (let i = 0; i < staticEventDates.length; i++) {
      const { day, month, year, eventType, text, eventYear } =
        staticEventDates[i];
      resultStaticEventDates.push(
        this.createEventDate({
          day: day,
          month: month,
          year: year,
  ...
  async createSourceEventDates() {
    ...
    return await new Promise(async (resolve, reject) => {
      ...
      lineReader = fileUtils.getFileLinesFromStream(
        pathService.pathDataModel.sourcePath
      );
      lineReader.on('line', (line) => {
        ...
        if (handleLineResult.returnValue) {
          switch (handleLineResult.eventType) {
            ...
            case EventTypeEnum.DEATHDAY: {
              sourceEventResult.sourceEventDates.push(
                handleLineResult.returnValue
              );
              break;
            }
            ...
            case EventTypeEnum.END_YEAR_TASK: {
              if (!handleLineResult.isSeparator) {
                ...
                sourceEventResult.commonTasks.push(
                  new CommonTaskModel({
            ...
            case EventTypeEnum.FUTURE_TASK: {
              if (
                !handleLineResult.isSeparator &&
                handleLineResult.returnValue.indexOf('=') === -1 &&
                ...
                const isDateTitle =
                  +handleLineResult.returnValue.replace(/\D/g, '') &&
                  /^\d/.test(handleLineResult.returnValue);
                if (isDateTitle) {
                  ...
                  sourceEventResult.futureTasks.push(
                    new FutureTasksModel({
                  ...
                  const futureTasksIndex =
                    sourceEventResult.futureTasks.findIndex(
                      ({ id }) => id === this.lastFutureDayId
                    );
                  if (futureTasksIndex > -1) {
                    sourceEventResult.futureTasks[futureTasksIndex].addEvent(
                      handleLineResult.returnValue
                    );
                  }
                }
        ...
        if (
          handleLineResult.line &&
          eventType &&
          (handleLineResult.isSeparator ||
            (eventType !== EventTypeEnum.COMPLETE_CANCEL_TASK &&
              eventType !== EventTypeEnum.FUTURE_TASK))
        ) {
  ...
  finalizeSourceEventResult(sourceEventResult) {
    ...
    sourceEventResult.dayTasks = this.filterSortTasks(
      sourceEventResult.commonTasks,
      EventTypeEnum.DAY_TASK
    );
    sourceEventResult.weekendOnToggleTasks = this.filterSortTasks(
      sourceEventResult.commonTasks,
      EventTypeEnum.WEEKEND_TASK
    );
    sourceEventResult.weekendOffToggleTasks = this.filterSortTasks(
      sourceEventResult.commonTasks,
      EventTypeEnum.WEEKEND_TOGGLE_TASK
    );
    sourceEventResult.endMonthTasks = this.filterSortTasks(
      sourceEventResult.commonTasks,
      EventTypeEnum.END_MONTH_TASK
    );
    sourceEventResult.halfYearTasks = this.filterSortTasks(
      sourceEventResult.commonTasks,
      EventTypeEnum.HALF_YEAR_TASK
    );
    sourceEventResult.yearTasks = this.filterSortTasks(
      sourceEventResult.commonTasks,
      EventTypeEnum.YEAR_TASK
    );
    sourceEventResult.endYearTasks = this.filterSortTasks(
      sourceEventResult.commonTasks,
      EventTypeEnum.END_YEAR_TASK
    );
  ...
  validateSourceEventType(data) {
    const { line } = data;
    let { eventType } = data;
    const validateSourceEventTypeResult =
      new ValidateSourceEventTypeResultModel(eventType);
    if (!line) {
    ...
    if (line[0] === separatorService.eventTypeSeparator) {
      validateSourceEventTypeResult.isBreakLine = true;
      switch (eventType) {
        case EventTypeEnum.INITIATE: {
          eventType = EventTypeEnum.SERVICE;
          break;
        }
        case EventTypeEnum.SERVICE: {
          eventType = EventTypeEnum.BIRTHDAY;
          break;
        }
        case EventTypeEnum.BIRTHDAY: {
          eventType = EventTypeEnum.DEATHDAY;
          break;
        }
        case EventTypeEnum.DEATHDAY: {
          eventType = EventTypeEnum.DATA;
          break;
        }
        case EventTypeEnum.WEEKEND_TOGGLE_TASK: {
          eventType = EventTypeEnum.END_MONTH_TASK;
          break;
        }
        case EventTypeEnum.END_MONTH_TASK: {
          eventType = EventTypeEnum.HALF_YEAR_TASK;
          break;
        }
        case EventTypeEnum.HALF_YEAR_TASK: {
          eventType = EventTypeEnum.YEAR_TASK;
          break;
        }
        case EventTypeEnum.YEAR_TASK: {
          eventType = EventTypeEnum.END_YEAR_TASK;
          break;
        }
        case EventTypeEnum.END_YEAR_TASK: {
          eventType = EventTypeEnum.FUTURE_TASK;
        ...
        case EventTypeEnum.FUTURE_TASK: {
          eventType = EventTypeEnum.START_EVENTS;
          break;
        }
        case EventTypeEnum.START_EVENTS: {
          eventType = EventTypeEnum.END;
          break;
        }
      }
    } else {
      ...
      switch (line) {
        ...
        case separatorService.dayTasksSeparator: {
          eventType = EventTypeEnum.DAY_TASK;
          break;
        }
        case separatorService.weekendTasksSeparator: {
          eventType = EventTypeEnum.WEEKEND_TASK;
          break;
        }
        case separatorService.weekendToggleTasksSeparator: {
          eventType = EventTypeEnum.WEEKEND_TOGGLE_TASK;
          break;
        }
        ...
        case separatorService.futureTasksSeparator: {
          eventType = EventTypeEnum.FUTURE_TASK;
          break;
        }
        case separatorService.startEventsSeparator: {
          eventType = EventTypeEnum.START_EVENTS;
          break;
        }
        default: {
  ...
  createSourceEvent(data) {
    ...
    const year = validationUtils.isIndexExists(dateParts, 2)
  ...
  createSourceEventText(data) {
    ...
    if (eventTemplate) {
      ...
      result = eventUtils.getDayEventTemplate(
        dictionaryCulture[`hebrew${eventTemplate}Day`],
        age
      );
    }
    return result;
  ...
  async validateSourceFile(data) {
    ...
    if (!fileUtils.isFilePath(filePath)) {
      throw new Error(
        `The parameter path ${parameterName} marked as file but it's a path of a directory: ${filePath} (1000004)`
      );
    }
    const extension = pathUtils.getExtname(filePath);
    if (extension !== '.txt') {
      throw new Error(
        `The parameter path ${parameterName} must be a TXT file but it's: ${extension} file (1000005)`
      );
    }
  }
  ...
  createCalendarDays(allEvents, futureEvents) {
    ...
    const dateStart = timeUtils.getCurrentDate([
    ...
    while (
      dateStart < dateEnd ||
      calendarDaysList.length < timeUtils.daysInYear
    ) {
      const { dateDay, dateMonth, dateYear } =
        timeUtils.getDateParts(dateStart);
      ...
      let calendarDayModel = new CalendarDayModel({
        ...
        eventDatesList: allEvents.filter(
          (e) => e.day === dateDay && e.month === dateMonth
        ),
      });
  ...
  createRepeatEventDate(data) {
    ...
    for (let i = 0; i < repeatEventDates.length; i++) {
      ...
      if (
        day === dateDay &&
        calendarDayModel.dayInWeek ===
          dictionaryCulture.englishDaysList[dayInWeek]
      ) {
        calendarDayModel.eventDatesList.push(
          this.createEventDate({
            day: dateDay,
            month: dateMonth,
            year: dateYear,
  ...
  async scanSourceFile() {
    ...
    lineReader.on('line', (line, lineNum = this.line_counter()) => {
      ...
      const last3Characters = line.slice(-3);
      if (
        firstCharacter !== separatorService.startLineCharacter ||
        last3Characters !== separatorService.scanEndLineCharacter
      ) {
        logUtils.log(
          `${lineNum}: | ${line} | ${line.split('').reverse().join('')}`
        );
      }
    });
  }
